...
class addForm(diesel.main):
    def __init__(self):
        ...
        # self.accountFilter.deleteLater()
        # self.monthFilter.deleteLater()
        # self.quarterFilter.deleteLater()
        ...
        self.selectSql = '''
            SELECT
                ...
                LEFT JOIN loads_bookkeeping ON loads_bookkeeping.idBookkeeping = bookkeeping.id
                -- Where condition statements
                WHERE bookkeeping.idCarrier = %s
                AND bookkeeping.idCategorie = 1
                AND loads_bookkeeping.idBookkeeping IS NULL
                ;'''
        self.requery()
    ...
    def requery(self): 
        if self.idCarrier:
            ...
            records = self.selectAll((self.idCarrier,))# will pass the cbo value - should be 
            self.list.requery(records,sizeVar=self.listFontSize, rowHeight= self.rowHeight)
